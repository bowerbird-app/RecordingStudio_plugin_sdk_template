...
**Engine Isolation**: Respect namespace isolation. All code should be properly namespaced under the engine's module (`RecordingStudioPluginSdkTemplate`). Use `isolate_namespace` patterns correctly.
...
**Engine-Specific**:
- Prefix all database tables with the engine name (e.g., `recording_studio_plugin_sdk_template_error_logs`)
...
- **Browser Simulation**: When modifying UI or flows (like `dashboard/index.html.erb`), explicitly verify the user experience:
  ...
  2. Navigate to the relevant dummy page (e.g., `/` or `/docs/config`).
...
When generating code, include file paths in comments (e.g., `# lib/recording_studio_plugin_sdk_template/configuration.rb`).
...
**Configuration**:
- Provide a configuration block: `RecordingStudioPluginSdkTemplate.configure do |config|`
