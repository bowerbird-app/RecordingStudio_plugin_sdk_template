...
class EngineTest < Minitest::Test
  def setup
    @original_configuration = RecordingStudioPluginSdkTemplate.instance_variable_get(:@configuration)
    RecordingStudioPluginSdkTemplate.instance_variable_set(:@configuration, RecordingStudioPluginSdkTemplate::Configuration.new)
  end

  def teardown
    RecordingStudioPluginSdkTemplate.configuration.hooks.clear!
    RecordingStudioPluginSdkTemplate.instance_variable_set(:@configuration, @original_configuration)
  end
  ...
  def test_before_and_after_initialize_initializers_run_hooks
    ...
    RecordingStudioPluginSdkTemplate.configuration.hooks.before_initialize { |_engine| before_called = true }
    RecordingStudioPluginSdkTemplate.configuration.hooks.after_initialize { |_engine| after_called = true }

    find_initializer("recording_studio_plugin_sdk_template.before_initialize").block.call(Object.new)
    find_initializer("recording_studio_plugin_sdk_template.after_initialize").block.call(Object.new)
  ...
  def test_load_config_merges_config_sources_and_runs_on_configuration_hook
    hook_called = false
    hook_payload = nil
    RecordingStudioPluginSdkTemplate.configuration.hooks.on_configuration do |cfg|
      hook_called = true
      hook_payload = cfg
    end
  ...
  def test_load_config_handles_errors_and_each_pair_fallback
    ...
    xcfg = Struct.new(:recording_studio_plugin_sdk_template).new(pair_config)
    app_config = Struct.new(:x).new(xcfg)
  ...
  def test_load_config_swallow_each_pair_errors
    ...
    xcfg = Struct.new(:recording_studio_plugin_sdk_template).new(bad_pair_config)
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      def config_for(_name)
        { api_key: "ok" }
  ...
  def test_apply_extension_initializers_register_active_support_on_load_callbacks
    ...
    RecordingStudioPluginSdkTemplate::Engine.stub(:config, config_stub) do
      find_initializer("recording_studio_plugin_sdk_template.apply_model_extensions").block.call
      find_initializer("recording_studio_plugin_sdk_template.apply_controller_extensions").block.call
    end
  ...
  def test_model_extension_initializer_skips_abstract_models
    ...
    with_temporary_nested_constant(:ActiveRecord, :Base, active_record_base) do
      RecordingStudioPluginSdkTemplate::Engine.stub(:apply_model_extensions, ->(model) { applied << model }) do
  ...
  def test_apply_model_extensions_adds_registered_methods_once
    ...
    RecordingStudioPluginSdkTemplate.configuration.hooks.extend_model(:ExampleRecord) do
      def template_extension_method
        :applied
      end
    end

    RecordingStudioPluginSdkTemplate::Engine.apply_model_extensions(model_class)
    RecordingStudioPluginSdkTemplate::Engine.apply_model_extensions(model_class)
  ...
  def test_apply_controller_extensions_matches_demodulized_name
    ...
    RecordingStudioPluginSdkTemplate.configuration.hooks.extend_controller(:DashboardController) do
      def template_controller_extension
        :applied
      end
    end

    RecordingStudioPluginSdkTemplate::Engine.apply_controller_extensions(controller_class)
  ...
  def test_apply_extensions_returns_without_target
    assert_nil RecordingStudioPluginSdkTemplate::Engine.send(:apply_extensions, nil, [])
  end
  ...
  def test_extension_keys_for_includes_demodulized_name
    ...
    assert_equal expected_keys, RecordingStudioPluginSdkTemplate::Engine.send(:extension_keys_for, namespaced)
  end
  ...
  def test_extension_keys_for_removes_duplicate_names
    ...
    assert_equal [:ReportsController], RecordingStudioPluginSdkTemplate::Engine.send(:extension_keys_for, plain)
  end
  ...
  def find_initializer(name)
    RecordingStudioPluginSdkTemplate::Engine.initializers.find { |initializer| initializer.name == name }
  end
end
