...
class RecordingStudioTemplateTest < ActiveSupport::TestCase
  ...
  test "workspace opts into accessible without the stripped example mixin" do
    workspace_source = File.read(Rails.root.join("app/models/workspace.rb"))

    refute_includes workspace_source, "Capabilities::Example"
    refute File.exist?(
      RecordingStudioPluginSdkTemplate::Engine.root.join("lib/recording_studio_plugin_sdk_template/capabilities/example.rb")
    )
