...
class CursorSkillFetchTest < ActiveSupport::TestCase
  test "fetch-skills extras come from the plugin catalog without hardcoded extra URLs" do
    root = RecordingStudioPluginSdkTemplate::Engine.root
  ...
  test "gem version stays 0.3.0 and gemspec still excludes .cursor" do
    ...
    spec = Gem::Specification.load(
      RecordingStudioPluginSdkTemplate::Engine.root.join("recording_studio_plugin_sdk_template.gemspec").to_s
    )
    cursor_files = spec.files.select { |path| path == ".cursor" || path.split("/").include?(".cursor") }

    assert_empty cursor_files

    tracked = `git -C #{RecordingStudioPluginSdkTemplate::Engine.root} ls-files -- .cursor/rules`
