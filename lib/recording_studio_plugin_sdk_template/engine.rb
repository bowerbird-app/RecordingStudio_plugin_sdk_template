  ...
  class Engine < ::Rails::Engine
    ...
    class << self
      ...
      def apply_extensions(target, extensions)
        ...
        applied = target.instance_variable_get(:@recording_studio_plugin_sdk_template_applied_extensions) || identity_hash
        ...
        target.instance_variable_set(:@recording_studio_plugin_sdk_template_applied_extensions, applied)
      end
    ...
    initializer "recording_studio_plugin_sdk_template.before_initialize", before: "recording_studio_plugin_sdk_template.load_config" do |_app|
    ...
    initializer "recording_studio_plugin_sdk_template.load_config" do |app|
      # Load config/recording_studio_plugin_sdk_template.yml via Rails config_for if present
      ...
      # Merge Rails.application.config.x.recording_studio_plugin_sdk_template if present
      if app.config.respond_to?(:x) && app.config.x.respond_to?(:recording_studio_plugin_sdk_template)
        xcfg = app.config.x.recording_studio_plugin_sdk_template
        if xcfg.respond_to?(:to_h)
          RecordingStudioPluginSdkTemplate.configuration.merge!(xcfg.to_h)
        else
          begin
      ...
      RecordingStudioPluginSdkTemplate.configuration.hooks.run(:on_configuration, RecordingStudioPluginSdkTemplate.configuration)
    end

    # Run after_initialize hooks
    initializer "recording_studio_plugin_sdk_template.after_initialize", after: "recording_studio_plugin_sdk_template.load_config" do |_app|
