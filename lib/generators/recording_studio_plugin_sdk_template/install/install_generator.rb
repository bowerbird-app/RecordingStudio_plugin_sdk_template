...
module RecordingStudioPluginSdkTemplate
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioPluginSdkTemplate engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_plugin_sdk_template",
        desc: "Route prefix used when mounting the engine"
      )

      def mount_engine
        route %(mount RecordingStudioPluginSdkTemplate::Engine, at: "#{options[:mount_path]}")
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioPluginSdkTemplate and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioPluginSdkTemplate and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioPluginSdkTemplate engine views for Tailwind CSS */",
